...
public class MovementRequest : ScriptableObject {
    ...
    public void LoadMovementRequests() {

        foreach(MovementType type in System.Enum.GetValues(typeof(MovementType))) {
            if (Resources.Load("MainCharacter/MovementRequests/" + type.ToString()) != null) {
                Debug.Log("not null");
            }
        }
    }
    ...
    protected virtual void OnEnable() {
        ...
        if (_type == MovementType.ShotgunMovement || _type == MovementType.MachineGunMovement) {
    ...
    private void AimLeft(float bodyXvel, float bodyYvel) {
        ...
        //SHOTGUN
        if (_type == MovementType.ShotgunMovement) {
            _xVel = !_grounded ? _recoil : _recoil * 0.5f;
        ...
        //MACHINEGUN
        else if (_type == MovementType.MachineGunMovement) {
