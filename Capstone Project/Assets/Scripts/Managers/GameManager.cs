...
public class GameManager : Singleton<GameManager>
{
    ...
    protected override void Awake()
    {
        ...
        EventSystemSingleton.Instance.GetComponent<StandaloneInputModule>();
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (scene.buildIndex != 0)
        {
            ...
            {
                SpawnPlayer();
                GameObject[] checkpoints = GameObject.FindGameObjectsWithTag(Tags.CheckpointTag);
                GameObject camera = GameObject.FindGameObjectWithTag(Tags.SmartCameraTag);
                ...
                foreach (GameObject ckpt in checkpoints)
                {
                    if (ckpt.GetComponent<CheckpointBehavior>().ID == SOSaveHandler.CheckpointID)
                    {
                        ...
                        camera.GetComponent<SmartCameraXPosition>().enabled = ckptBehavior.smartXEnabled ? true : false;
                        camera.GetComponent<SmartCameraYPosition>().enabled = ckptBehavior.smartYEnabled ? true : false;
                    }
                }
            }
        }
