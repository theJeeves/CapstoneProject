...
public class SmartCameraYPosition : MonoBehaviour {
    ...
    //        // CHECK IF THE PLAYER IS CLOSE TO THE TOP OF THE SCREEN SO WE CAN ADJUST UPWARDS
    //        if (_currPlayerPos.y >= 0.7f) {
    //            _movingUp = true;
    //            _adjusting = true;
    //            _startTime = 0.0f;
    //        }
    //        // IF THE CAMERA NEEDS TO ADJUST, KEEP ADJUSTING DOWN UNTIL THE PLAYER IS IN THE CENTER OF THE SCREEN.
    //        else if (_adjusting) {
    //            if (_startTime == 0.0f) {
    //                _startTime = Time.time;
    //            }
